...
const totalContributions = GAMES_JSON.reduce( (sum, game) => {return sum + game.backers}, 0);
...
function filterUnfundedOnly() {
    ...
    const gamesNotMetGoal = GAMES_JSON.filter( (games) => {return games.pledged < games.goal});
...
function filterFundedOnly() {
    ...
    const gamesMetGoal = GAMES_JSON.filter( (games) => {return games.pledged > games.goal});
...
let displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames} games. Currently, ${
  gamesNotMetGoal.length > 0
    ? `${gamesNotMetGoal.length} game${gamesNotMetGoal.length > 1 ? 's' : ''} remain${gamesNotMetGoal.length > 1 ? '' : 's'} unfunded. 
...
let [{ name: firstName }, { name: secondName }, ...restOfGames] = sortedGames;
